...
class FilePlayer extends import_react.Component {
  ...
  load(url) {
    ...
    if (this.shouldUseHLS(url)) {
      (0, import_utils.getSDK)(hlsSdkUrl || HLS_SDK_URL.replace("VERSION", hlsVersion), HLS_GLOBAL).then((Hls) => {
    ...
    if (this.shouldUseDASH(url)) {
      (0, import_utils.getSDK)(dashSdkUrl || DASH_SDK_URL.replace("VERSION", dashVersion), DASH_GLOBAL).then((dashjs) => {
    ...
    if (this.shouldUseFLV(url)) {
      (0, import_utils.getSDK)(flvSdkUrl || FLV_SDK_URL.replace("VERSION", flvVersion), FLV_GLOBAL).then((flvjs) => {
